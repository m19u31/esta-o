...
#define LDRPIN 0  // Pino Analógico
//sensor temp e hum
#define DHTPIN 2     /// Digital 2
#define DHTTYPE DHT22   // DHT 22  (AM2302), AM2321
#define ANEMOMETRO_PIN   4     // Digital 4
...
double volumeMedio,soma;

// CONSTANTES DE CALIBRAÇÃO
// 1 rev/segundo = 1 mph =2.4011 kph
...
#define PERIODO_ANEMOMETRO  30000
#define PERIODO_PLUVIOMETRO  30000
...
IPAddress SERVER = IPAddress(10,10,10,1); // IP Adress (or name) of server to dump data to
int SERVER_PORT = 80;
const String URI = "/meteorologia-api/v1/index.php/submitreading";
const String AUTH = "12345678901234567890";
uint8_t MAC[6] = {0x00,0x01,0x02,0x03,0x04,0x05};
IPAddress IP = IPAddress(10,10,10,100);
...
Thread printThread, sensor1Thread, sensor2Thread, sensor3Thread, sensor4Thread, sensor5Thread;
...
//////////////////////////////////////////////////////////////////////////////
//inserir aqui clases pluviometro e anemometro ///
/*
...
void setup() {
  ...
  //Serial.begin(9600);   //tava comentado antes
  dht.begin();
  if (!bmp180.begin()) 
  {
...
/////////////////////////////////////////////////////////////////////
  ...
  sensor4Thread = Thread();
  sensor5Thread = Thread();
  
  printThread.setInterval(30000); //5 min //300000
  sensor1Thread.setInterval(30000);
  sensor2Thread.setInterval(30000);//Fixed to 1000 
  sensor3Thread.setInterval(30000);//Fixed to 1000 
  sensor4Thread.setInterval(30000);//Fixed to 1000 deixar mesmo valor ver PERIODO_PLUVIOMETRO;
  sensor5Thread.setInterval(30000);//Fixed to 1000 deixar mesmo valor ver PERIODO_ANEMOMETRO;
  ...
  Sensor sensor4(1);
  sensor4.name = "Pluviometro";  /////////////////////
  sensor4.probe[0] = "Quantidade de chuva";
  sensors.push_back(sensor4);
  Sensor sensor5(1);
  sensor5.name = "Anemometro"; 
  sensor5.probe[0] = "Velocidade vento";
  sensors.push_back(sensor5);   //////////////////////
  ...
  sensor4Thread.onRun(calcQuantidadeChuva); //pluviometro
  sensor5Thread.onRun(calcVelocidadeVento); //anemometro
...
void loop() {
  ...
  if(sensor3Thread.shouldRun())
    sensor3Thread.run();
  if(sensor4Thread.shouldRun())
    sensor4Thread.run();
  if(sensor5Thread.shouldRun())
    sensor5Thread.run();    
}
